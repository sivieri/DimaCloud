...
@Api(name = "noteendpoint", namespace = @ApiNamespace(ownerDomain = "sivieri.me", ownerName = "sivieri.me", packagePath = "dimatodos"))
public class NoteEndpoint {
	...
	public CollectionResponse<Note> listNote(@Nullable @Named("cursor") String cursorString, @Nullable @Named("limit") Integer limit) {
		...
		return CollectionResponse.<Note> builder().setItems(execute).setNextPageToken(cursorString).build();
	}
	...
	public Note insertNote(Note note) {
		...
		try {
			/*
			 * if (containsNote(note)) { throw new
	...
	private boolean containsNote(Note note) {
		...
		try {
			Note item = mgr.find(Note.class, note.getKey());
			if (item == null) {
				contains = false;
			}
		}
